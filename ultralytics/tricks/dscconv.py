...
class DySnakeConv(nn.Module):
    def __init__(self, inc, ouc, k=3, e = 0.5) -> None:
        super().__init__()
        #self.c = int(ouc * e)

        self.conv_0 = Conv(2*ouc, ouc, k)
        self.conv_x = DSConv_pro(inc, ouc, kernel_size=k, morph=0)
        self.conv_y = DSConv_pro(inc, ouc, kernel_size=k, morph=1)

    def forward(self, x):
        return x + self.conv_0(torch.cat([self.conv_x(x), self.conv_y(x)], dim=1))
...
class DSConv_pro(nn.Module):
    def __init__(
        ...
        self.extend_scope = extend_scope #1.0
        self.morph = morph #0 or 1
...
def get_coordinate_map_2D(
    ...
    y_offset_, x_offset_ = torch.split(offset, kernel_size, dim=1) # [B, K, W, H], [B, K, W, H]

    y_center_ = torch.arange(0, width, dtype=torch.float32, device=device) #一维张量 [W, ]
    y_center_ = einops.repeat(y_center_, "w -> k w h", k=kernel_size, h=height) # 增加维度 [K, W, H]

    x_center_ = torch.arange(0, height, dtype=torch.float32, device=device)
    x_center_ = einops.repeat(x_center_, "h -> k w h", k=kernel_size, w=width)# 增加维度 [K, W, H]
    ...
    if morph == 0:
        ...
        """
        y_spread_ = torch.zeros([kernel_size], device=device) # 全为0的张量，一维，大小为[K, ]
        x_spread_ = torch.linspace(-center, center, kernel_size, device=device) #创建一维张量，从-1到1，包含3个元素。[K, ]

        y_grid_ = einops.repeat(y_spread_, "k -> k w h", w=width, h=height)# 增加维度 [K, W, H]
        x_grid_ = einops.repeat(x_spread_, "k -> k w h", w=width, h=height)# 增加维度 [K, W, H]
