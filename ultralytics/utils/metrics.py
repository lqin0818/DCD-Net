...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, FCIoU=False, eps=1e-7):
    """
    ...
    if FCIoU:
        d = 0.05
        u = 0.95
        iou_focaler = ((iou - d) / (u - d)).clamp(0, 1)  # default d=0.00,u=0.95

    if CIoU or DIoU or GIoU or FCIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or FCIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw.pow(2) + ch.pow(2) + eps  # convex diagonal squared
            rho2 = (
                (b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)
            ) / 4  # center dist**2
            if CIoU or FCIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                if FCIoU:
                    return iou - (rho2 / c2 + v * alpha) + iou - iou_focaler #FCIoU
